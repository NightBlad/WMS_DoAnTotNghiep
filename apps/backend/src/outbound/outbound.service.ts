...
export class OutboundService implements OnModuleInit {
  ...
  private async persistDetails(orderId: string, items: OutboundItemDto[], branchCode?: string) {
    ...
    for (const item of items) {
      ...
      if (!product) {
        const itemIdentifier = (item.productName || item.productSku || item.productId || 'Không rõ').trim();
        throw new BadRequestException(
          `Mặt hàng "${itemIdentifier}" không tồn tại trong cơ sở dữ liệu! Vui lòng kiểm tra và chọn đúng mặt hàng trong danh mục sản phẩm trước khi lưu phiếu xuất.`
        );
      }
      ...
      const detail = this.detailRepo.create({
        ...
        product,
        productSku: product.internalSku || item.productSku?.trim() || undefined,
        productName: product.name || item.productName?.trim() || undefined,
        unit: product.unit || item.unit?.trim() || 'Cái',
